...
public class DefaultEnvironmentalCategory implements EnvironmentalCategory, Introspectable {
  ...
  public void externalise(AbsObject absObj, Ontology onto) throws OntologyException {
    try {
      ...
      abs.set(TaxonomyOntology.ENVIRONMENTALCATEGORY_DEFINITION, (AbsTerm) onto.fromObject(getDefinition()));
      abs.set(TaxonomyOntology.ENVIRONMENTALCATEGORY_OWNS, (AbsTerm) onto.fromObject(getOwns()));
      abs.set(TaxonomyOntology.ENVIRONMENTALCATEGORY_TERM, (AbsTerm) onto.fromObject(getTerm()));
  ...
  public void internalise(AbsObject absObj, Ontology onto) throws UngroundedException, OntologyException {
    try {
      AbsConcept abs = (AbsConcept) absObj;
      definition = (String)onto.toObject(abs.getAbsObject(TaxonomyOntology.ENVIRONMENTALCATEGORY_DEFINITION));
      owns = (List)onto.toObject(abs.getAbsObject(TaxonomyOntology.ENVIRONMENTALCATEGORY_OWNS));
      term = (String)onto.toObject(abs.getAbsObject(TaxonomyOntology.ENVIRONMENTALCATEGORY_TERM));
     } catch (ClassCastException cce) {
